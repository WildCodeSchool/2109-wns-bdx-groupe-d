	...
	return (
		<div className="detail-project-container">
			...
			<div className="project-collaborators flex items-start justify-start">
				...
				{collaboratorsProject &&
					collaboratorsProject.map((collaborator) => (
						<img
							key={collaborator.id}
							className="rounded-full h-8 w-8 object-cover mx-2"
			...
			<img
				...
				src={data.getProjectById.projectPictureName ? `/images/${data.getProjectById.projectPictureName}` : smiley}
				alt="collabo 1"
			/>
			<Carousel>
				{imagesProject &&
					imagesProject.map((image) => (
						<CarouselItem>
							<img key={image.id} className="object-none object-center" src={image.url} alt="collabo 1" />
						</CarouselItem>
					))}
			</Carousel>
			...
			<div className="relative overflow-x-auto my-6 rounded-lg">
				<table className="w-full text-sm text-center text-gray-500 dark:text-gray-400 ">
					...
					<tbody>
						{projectIssus &&
							projectIssus.slice(0, 5).map((projectIssu) => (
								<tr key={projectIssu.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
									<td
										scope="row"
										className="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap text-lg"
									>
										<NavLink
											to={'/issue/' + projectIssu.id}
											className="rounded-full py-1 px-4 bg-wildmine_black text-blue_green_flash"
										>
									...
									<td className="px-6 py-4 font-serif text-lg">
										<Statut value={projectIssu.statut} />
									</td>
									...
									<td className="px-6 py-4 space-x-2 flex items-center justify-center">
										{projectIssu.avatars &&
											projectIssu.avatars.map((avatar) => (
												<img
													key={avatar}
						...
						{showFiveTickets
							? projectIssus &&
							  projectIssus.slice(6, 10).map((projectIssu) => (
									<tr key={projectIssu.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
										<td
											scope="row"
											className="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap text-lg"
